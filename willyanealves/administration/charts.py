...
def barchart_billing_profit(date_bp, total, profit):
    ...
    fig.add_scatter(x=df['date'], y=df['profit'], hovertemplate="<b>R$%{y:.2f}</b><br>" +
                    "Data: %{x|%d/%m/%Y}<br>" +
                    "Total: R$%{y:.2f}<br>" +
                    "<extra></extra>")
    fig.update_layout(
    ...
    fig.update_traces(hovertemplate="<b>R$%{y:.2f}</b><br>" +
                      "Data: %{x|%d/%m/%Y}<br>" +
                      "Total: R$%{y:.2f}<br>" +
                      "<extra></extra>")
    fig.update_xaxes(tickformat='%d/%b', tickfont_size=8, tickmode='linear')

    return fig.to_html(config={'modeBarButtonsToRemove': [
